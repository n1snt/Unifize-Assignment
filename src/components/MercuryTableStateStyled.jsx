...
const MercuryTableStateStyled = ({ data }) => {
                ...
                <tbody>
                    {
                        data.map((item, index) => {
                            ...
                            return (<TableItem data={item} index={index} showDate={showDate} formattedDate={formattedDate} />);
                        })
                    }
                </tbody>
            </table>
    ...
    function TableItem({ data, index, showDate, formattedDate }) {
        return (
            <tr class="bg-white border-b dark:bg-zinc-800 dark:border-gray-700 border-gray-200 hover:bg-zinc-50 dark:hover:bg-zinc-600"
                ...
                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                    {(hoveredRow === index || showDate) && (
                        <span data-testid={`date-cell-${index}`}>{formattedDate}</span>
                    )}
                </th>
                <td class="px-6 py-4">
                    {data.description}
...
export default MercuryTableStateStyled;